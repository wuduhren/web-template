// -----------------------------------------------------------------------------
var _URL_LOGIN = 'index.php'
var _URL_HOME = 'home.php'
var _URL_EXPIRE = 'expire.html'
var _URL_MAINTENANCE = 'maintenance.html'

// -----------------------------------------------------------------------------
// loading
var _SHOW_LOADING_COUNTER = 0
function show_loading() {
    _SHOW_LOADING_COUNTER += 1
    if (_SHOW_LOADING_COUNTER==1) $('#loading').show()
}

function hide_loading(delay, force) {
    setTimeout(function(){
        _SHOW_LOADING_COUNTER -= 1
        if (force==true) _SHOW_LOADING_COUNTER = 0
        if (_SHOW_LOADING_COUNTER<=0) {
            _SHOW_LOADING_COUNTER = 0
            $('#loading').hide()
        }
    }, delay || 0)
}
    ...
    setTimeout(function(){
        $.ajax({
            ...
            success:function(response, status, xhr) {
                ...
                if (code==2) {
                    if (opt.complete) opt.complete()
                    let url = (payload[1]) ? payload[1] : _URL_MAINTENANCE
                    if (opt.debug==true) {
                ...
                if (code==3) {
                    if (opt.complete) opt.complete()
                    let url = (payload[1]) ? payload[1] : _URL_EXPIRE
                    if (opt.debug==true) {
                        if (confirm(url)==true) window.location=url
                    }
                    else {window.location=url}
...
function hide_warning(selector) {
    hide_loading(200)
    setTimeout(function(){
        $(selector || '#warning').html('').hide()    
    }, 200)
}
// -----------------------------------------------------------------------------
// show_notify
function show_notify(msg, dealy) {
    setTimeout(() => {
        $.notify({message:msg}, {type:'warning', delay:100, timer:500, z_index:9999, placement:{from:'bottom', align:'left'}})    
    }, dealy || 0);
}
